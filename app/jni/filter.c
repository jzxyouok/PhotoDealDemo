...
void applyMasic(Bitmap* bitmap) {
	...
	unsigned int width = (*bitmap).width;
	unsigned int height = (*bitmap).height;
	register unsigned int i,j,n,m;
	//register unsigned int grey;
	register unsigned char r, g, b;
    //the size of masic
    unsigned int offset = 10;
    for(i=0;i<height;i += offset){
        for(j=0;j<width;j += offset){
            r = red[i*width + j];
            g = green[i*width + j];
            b = blue[i*width + j];
            for(n=0;n<offset && (i+n)<height;n++){
                for(m=0;m<offset && (j+m)<width;m++){
                    red[i*width + j + n*width+ m] = r;
                    green[i*width + j + n*width+ m] = g;
                    blue[i*width + j + n*width+ m] = b;
                }
            }
        }
    }
...
int applyBlur(Bitmap* bitmap) {
    ...
    unsigned char* srcRed,srcGreen,srcBlue;
    ...
    memcpy(blurRed, red, length);
    memcpy(blurGreen, green, length);
    memcpy(blurBlue, blue, length);

    resultCode = stackBlur(&blurRadius, blurRed, blurGreen, blurBlue, &width, &height, red, green, blue);
